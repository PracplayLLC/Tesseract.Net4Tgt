...
namespace Tesseract.Tests
{
    ...
    public class AnalyseResultTests
    {
        ...
        public void Init()
        {
            ...
            engine = new TesseractEngine(TestTESSDATA.Path, "eng", EngineMode.Default);
        }
         ...
         */
        public void AnalyseLayout_RotatedImage()
        {
            RotateFlipType rotation = RotateFlipType.Rotate90FlipNone;
            using (var img = new Bitmap(@".\data\phototest.tif")) {
                if (rotation!=null) img.RotateFlip(rotation);
            ...
            )
        {
            ...
            using (var img = Pix.LoadFromFile(@".\data\phototest.tif")) {
                using (var rotatedPix = img.Rotate((float)Math.PI)) {
                    using (var page = engine.Process(rotatedPix, pageSegMode)) {
                        ...
                        Assert.AreEqual(Orientation.PageDown, orientation);
        ...
        [TestCase(270f)]
         */
        public void DetectOrientation_RotatedImage()
        {
            float rotation = 270f;
            using (var img = Pix.LoadFromFile(@".\data\phototest.tif")) {
        ...
        public void GetImage(
            ...
            [Values(0, 3)]*/)
        {
            PageIteratorLevel level = PageIteratorLevel.Block;
            int padding = 3;
            using (var img = new Bitmap(@".\data\phototest.tif")) {
                using (var page = engine.Process(img)) {
                    using (var pageLayout = page.GetIterator()) {
