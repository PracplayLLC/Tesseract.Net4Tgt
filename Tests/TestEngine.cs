...
namespace Tesseract.Tests
{
	...
	{
        static string TDP = TestTESSDATA.Path;
		[Test]
			...
			using (var engine = new TesseractEngine(TDP, "eng", EngineMode.Default)) {
				using (var img = Pix.LoadFromFile("./data/phototest.tif")) {
					using (var page = engine.Process(img, Rect.FromCoords(0, 0, img.Width, 188))) {
                        ...
                        Assert.AreEqual(expectedTextRegion1, region1Text);
                        ...
                        Assert.AreEqual(expectedTextRegion2, region2Text);
...
")));
		...
		{
			using (var engine = new TesseractEngine(TDP, "eng", EngineMode.Default)) {
				using (var img = Bitmap.FromFile("./data/phototest.tif")) {
		...
		{
            var tessDataPath = TDP; // Path.Combine(Environment.CurrentDirectory, @"tessdata\");
			using (var engine = new TesseractEngine(tessDataPath, "eng", EngineMode.Default, "bazzar")) {
				...
				if (engine.TryGetStringVariable("user_words_suffix", out user_patterns_suffix)) {
                    Assert.AreEqual("user-words", user_patterns_suffix);
		...
		public void Initialise_CanPassInitVariables()
		{
			...
			using (var engine = new TesseractEngine(TDP, "eng", EngineMode.Default, Enumerable.Empty<string>(), initVars, false)) {
				bool loadSystemDawg;
		...
		{
			using (var engine = new TesseractEngine(TDP, "rus", EngineMode.Default)) {
			}
		}
		...
		{
			Environment.SetEnvironmentVariable("TESSDATA_PREFIX", TDP);
		...
		[Test]
		public void CanSetBooleanVariable()
		{
